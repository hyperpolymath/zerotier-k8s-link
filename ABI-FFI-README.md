...
# ZEROTIER_K8S_LINK ABI/FFI Documentation
...
│  generated/abi/zerotier-k8s-link.h                │
└─────────────────┬───────────────────────────┘
                  │
                  │ imported by
                  ▼
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libzerotier-k8s-link.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── zerotier-k8s-link.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── zerotier-k8s-link.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/zerotier-k8s-link.h
```
...
```c
#include "zerotier-k8s-link.h"

int main() {
    void* handle = zerotier-k8s-link_init();
    if (!handle) return 1;

    int result = zerotier-k8s-link_process(handle, 42);
    if (result != 0) {
        const char* err = zerotier-k8s-link_last_error();
...
Compile with:
```bash
gcc -o example example.c -lzerotier-k8s-link -L./zig-out/lib
```

### From Idris2

```idris
import ZEROTIER_K8S_LINK.ABI.Foreign
...
```rust
#[link(name = "zerotier-k8s-link")]
extern "C" {
    fn zerotier-k8s-link_init() -> *mut std::ffi::c_void;
    fn zerotier-k8s-link_free(handle: *mut std::ffi::c_void);
    fn zerotier-k8s-link_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = zerotier-k8s-link_init();
        assert!(!handle.is_null());

        let result = zerotier-k8s-link_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libzerotier-k8s-link = "libzerotier-k8s-link"

function init()
    handle = ccall((:zerotier-k8s-link_init, libzerotier-k8s-link), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:zerotier-k8s-link_process, libzerotier-k8s-link), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:zerotier-k8s-link_free, libzerotier-k8s-link), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/zerotier-k8s-link.h
   ```
