...
*zerotier-k8s-link* has achieved *CRG Grade C* per the Hyperpolymath
Testing & Benchmarking Taxonomy (v1.0).
...
** network.ncl contains ZeroTier config (network_id, api_token, IP
pools, routes)
...
** All 6 manifest types exist (configmap, daemonset, namespace,
networkpolicy, secret, servicemonitor)
...
** 4 bash scripts exist (authorize-nodes.sh, configure-routes.sh,
health-check.sh, join-network.sh)
** setup.sh exists with shell shebang
** 4 validation hooks exist (validate-codeql.sh,
validate-permissions.sh, validate-sha-pins.sh, validate-spdx.sh)
* *ABI and manifest validity*
** 3 Idris2 ABI files exist (Layout.idr, Types.idr, Foreign.idr) with
module declarations
** All Kubernetes manifests are non-empty YAML with apiVersion and kind
fields
...
* *Format validation*
** ZeroTier network ID is either placeholder (CHANGEME_NETWORK_ID) or 16
hex characters
...
*Total coverage*: 79 tests + 11 benchmarks = comprehensive CRG C
validation
...
[arabic]
. *Integration tests* — Deploy to K8s and verify network connectivity
. *Mutation tests* — Verify test sensitivity to code changes
. *Load tests* — ZeroTier throughput under traffic
. *Chaos tests* — Network failures, node restarts, credential rotation
. *Upgrade tests* — Zero-downtime upgrades of ZeroTier daemon
. *Target coverage* — 6 specific targets (see RSR template)

=== References

* *Testing Taxonomy*: Hyperpolymath Testing & Benchmarking Taxonomy v1.0
* *RSR Standard*: Rhodium Standard Repositories
