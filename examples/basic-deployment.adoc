...
This guide shows how to deploy ZeroTier to a Kubernetes cluster for
...
[arabic]
. Log into https://my.zerotier.com
. Create a network (or use existing)
. Note the Network ID (16 characters)
. Generate an API token: Account → API Access Tokens
...
Option B: Manual authorization in ZeroTier Central: 1. Go to your
network page 2. Scroll to "`Members`" section 3. Check the boxes next to
your K8s nodes 4. Save
